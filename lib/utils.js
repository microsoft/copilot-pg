...
exports.toCSV = async function(fileName, results){
  ...
  this.writeToTemp(fileName, csv)
  return fileName;
}

exports.toJSON= function(fileName, results){
  
  //this.writeToTemp(`${fileName}.json`,JSON.stringify(results, null, 2))
  this.writeToTemp(fileName,JSON.stringify(results, null, 2))
}
//this doesn't seem to work consistently
exports.toText= function(fileName, results){
  const ascii = this.jsonToAscii(fileName, results);
  //this.writeToTemp(`${fileName}.txt`,ascii)
  this.writeToTemp(fileName,ascii)
}
...
exports.tempfileInEditor = function(fileName){
  const tempDir = path.resolve(__dirname,"../temp");
  const tempFile =  path.resolve(tempDir, fileName);
  const isOpen =  vscode.window.activeTextEditor && vscode.window.activeTextEditor.document.fileName === tempFile;
  return isOpen
}
exports.writeToTemp = async function(fileName, val){
  ...
  //make sure we don't double things up
