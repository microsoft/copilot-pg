...
class PG extends Participant{
  constructor(){
    ...
    this.formatPrompt = async function(prompt){
      ...
      try{
        ...
        Editor.writeAndShowFile("schema.json", schema);
  ...
  async setCommands(){
    ...
    this.commands.ddl = Command.createCommand("ddl", async (request, stream) => {
      const schema = await this.db.getSchema();
      const prompt = `You are a savvy Postgres DBA tasked with creating a database schema. Please provide the DDL for "${request.prompt.trim()}". Be detailed with your answer and explanation, and always return SQL examples. Any DDL you create should comply with this schema: ${schema}`;
