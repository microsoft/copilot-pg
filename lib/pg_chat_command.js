...
class PGChatCommand {
  ...
  async chatWithCopilot(rawPrompt, token, cb){
    ...
    const prompt = `Create a select query for a PostgreSQL database for ${rawPrompt}. The reference schema for this database is ${schema}.`;

    this.messages.push(new vscode.LanguageModelChatUserMessage(prompt))
